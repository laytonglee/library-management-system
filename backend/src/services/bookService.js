...
async function addBookCopy(bookId, { barcode, location, status, actorId, ipAddress } = {}) {
  return withSerializableTransaction(async (tx) => {
    ...
    await auditLogger.log(tx, {
      ...
      targetType: "book",
      targetId: bookId,
      details: { copyId: copy.id, barcode: barcode ?? null, location: location ?? null },
      ipAddress,
    });
...
async function updateBookCopy(bookId, copyId, data = {}, { actorId, ipAddress } = {}) {
  return withSerializableTransaction(async (tx) => {
    ...
    await auditLogger.log(tx, {
      ...
      targetType: "book",
      targetId: bookId,
      details: { copyId, before: { status: existing.status }, after: { barcode: data.barcode ?? null, location: data.location ?? null, status: data.status ?? null } },
      ipAddress,
    });
